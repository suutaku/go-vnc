...
func (g *Gstreamer) Start(width, height int) error {
	logrus.Debug("Building gstreamer pipeline for display connection")
	...
	decodebin.Connect("pad-added", func(self *gst.Element, srcPad *gst.Pad) {
		logrus.Debug("Decodebin pad added, linking pipeline")
		...
		if err := runAllUntilError([]func() error{
			func() error { return videoscale.SetProperty("sharpen", 1.0) },
			// func() error { return videoscale.SetProperty("method", "nearest-neighbour") }, // Use nearest neighbor - significantly cheaper CPU
		...
		logrus.Debug("Syncing new element states with parent pipeline")
		...
		sink.SetCallbacks(&app.SinkCallbacks{
			NewSampleFunc: func(self *app.Sink) gst.FlowReturn {
				...
				logrus.Debug("Received new frame on the pipeline, decoding")
				...
				if !ok {
					logrus.Debug("Client is behind on frames, could not push to channel")
				} else {
					logrus.Debug("Successfully queued frame for processing")
				}
		...
		if ret := srcPad.Link(queue.GetStaticPad("sink")); ret != gst.PadLinkOK {
			logrus.Error("Could not link src pad to pipeline")
		}
	})
...
func getScreenCaptureElement() (elem *gst.Element, err error) {
	...
	case "windows":
		logrus.Debug("Detected Windows, using gdiscreencapsrc")
	...
	case "darwin":
		logrus.Debug("Detected macOS, using avfvideosrc")
	...
	default:
		logrus.Debug("Detected Linux, using ximagesrc")
...
func logPipelineErr(err error) {
	logrus.Error("[go-gst-error] ", err.Error())
}
